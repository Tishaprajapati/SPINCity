...
CREATE TABLE `ride_analytics` (
  ...
  `route_coordinates` text COLLATE utf8mb4_unicode_ci,
  ...
  KEY `idx_peak_hour` (`peak_hour`),
  CONSTRAINT `ride_analytics_ibfk_1` FOREIGN KEY (`transaction_id`) REFERENCES `rental_transaction` (`transaction_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `ride_analytics_ibfk_2` FOREIGN KEY (`customer_id`) REFERENCES `customer` (`customer_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `ride_analytics_chk_1` CHECK ((`distance_traveled` >= 0)),
  CONSTRAINT `ride_analytics_chk_2` CHECK ((`average_speed` >= 0)),
  CONSTRAINT `ride_analytics_chk_3` CHECK ((`carbon_saved` >= 0)),
  CONSTRAINT `ride_analytics_chk_4` CHECK ((`calories_burned` >= 0))
...
-- Dump completed on 2026-03-19 23:22:04

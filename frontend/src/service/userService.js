...
const userService = {
  ...
  const response = await fetch(
    `https://spincity.onrender.com/api/employee/payment-status/${transactionId}?status=${status}`,
    {
      method: "PUT",
...
startRental: async (customerId, cycleId, pickupStationId, returnStationId, expectedReturnTime,bookedAmount) => {
  const token = getTokenForPath(window.location.pathname);
  const response = await fetch("https://spincity.onrender.com/api/user/rentals/start", {
    method: "POST",
    headers: {
  ...
  getActiveRental: async (customerId) => {
    const token = getTokenForPath(window.location.pathname);
    const response = await fetch(
      `https://spincity.onrender.com/api/user/rentals/active/${customerId}`,
      {
        headers: { Authorization: `Bearer ${token}` },
      },
    );
 ...
 endRental: async (rentalId, returnStationId) => {
  const token = getTokenForPath(window.location.pathname);
  const response = await fetch(
    `https://spincity.onrender.com/api/user/rentals/end/${rentalId}`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
checkRideStatus: async (transactionId) => {
  const token = getTokenForPath(window.location.pathname);
  const response = await fetch(
    `https://spincity.onrender.com/api/employee/ride-status/${transactionId}`,
    { headers: { Authorization: `Bearer ${token}` } }
  );
...
checkApprovalStatus: async (transactionId) => {
  const token = getTokenForPath(window.location.pathname);
  const response = await fetch(
    `https://spincity.onrender.com/api/employee/approval-status/${transactionId}`,
    {
      headers: {
        Authorization: `Bearer ${token}`,
      },
